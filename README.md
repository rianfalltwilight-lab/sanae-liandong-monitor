...
监控用户指定的五家已确认店铺，并加入麻豆科技上货货架，匹配所有价格的短时速刷 team5x，向 QQ 群 **1092470719** 发送，并将同一提醒独立私发给 QQ **2731538103**；两路分别持久化回执，私聊失败不会重发已成功的群消息。仅发送上架和最后五个提醒。上架包括新商品、重新上架、售罄后补货；最后五个指库存从大于 5 降到 1–5，跨过门槛时只提醒一次。每条通知附公开购买链接及本轮健康店铺中已知有货商品的最低单价。不同套餐时长不等，最低单价不等于最低每小时价格。
