...
监控用户指定的六家已确认店铺，并加入麻豆科技上货货架，匹配所有价格的短时速刷 team5x。QQ 群 **1092470719** 只接收特惠车提醒（有货、单价严格低于 ¥40、明确提供 2FA，并且是新上架/补货或进入最后五个），并艾特 **3294692833、1920924896**；QQ 群 **837056300** 接收全部新上架/补货和进入最后五个的实时提醒，不艾特任何人；QQ **2731538103** 独立私聊接收全部提醒。三个目的地分别持久化发送回执，一路失败只重试该路，不会重发已成功的另外两路。启用新群前已经排队的消息标记为已处理，不会补发到新群。上架包括新商品、重新上架、售罄后补货；最后五个指库存从大于 5 降到 1–5，跨过门槛时只提醒一次。每条通知附公开购买链接及本轮健康店铺中已知有货商品的最低单价。不同套餐时长不等，最低单价不等于最低每小时价格。
...
- **醒目提醒**：上述两类通知中的商品若有货、单价严格低于 ¥40 且明确提供 2FA，显示 `🚨【低价2FA速刷提醒｜低于¥40】`。该消息在群 **1092470719** 真实艾特 **3294692833、1920924896**，转发到群 **837056300** 和私聊时均不附加艾特消息段。艾特规则不独立产生降价或普通补量消息。¥40 整、库存未知/无货、未明确提供 2FA 均不触发艾特。
- OneBot 固定两个群目标，商品内容使用纯文字段，只有群 **1092470719** 的特惠规则可附加两位固定账号的 `at` 消息段，商家标题不能插入 CQ 指令。每个目的地只有在 HTTP 成功、业务成功并返回 message_id 后才单独记为已发。
- 状态、待发消息和三个目的地的回执持久化；单个目的地失败时，另外两个目的地仍继续发送，后续只补齐失败目的地。超过 120 秒或内容已变化的通知按原事件类型刷新价格/库存，并保留已成功目的地的回执。最后五个通知在补量到 5 个以上或售罄后取消；迟发上架消息标记“延迟送达”。QQ 不提供幂等发送事务，极端情况下发送成功后进程立即崩溃，重启可能在该目的地重复一次。
...
本私有部署固定特惠群 **1092470719**、全量实时群 **837056300**、私聊 **2731538103** 及两位艾特对象，代码有对应限制。配置字段分别为 `group_id`、`all_alert_group_id` 与 `private_forward_qq`。改用其它目的地时须同时调整 `shop_monitor.py` 的目标限制和配置。`install-monitor.ps1`、`start-monitor.ps1`、`stop-monitor.ps1` 面向下述 Windows 部署路径；在其它机器使用前需调整脚本路径。`--dry-run` 只生成预览；省略它会使用实际发群通道。
