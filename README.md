...
监控用户指定的四家店铺，匹配所有价格的短时速刷 team5x，向 QQ 群 **1092470719** 仅发送上架和最后五个提醒。上架包括新商品、重新上架、售罄后补货；最后五个指库存从大于 5 降到 1–5，跨过门槛时只提醒一次。每条通知附公开购买链接及本轮健康店铺中已知有货商品的最低单价。不同套餐时长不等，最低单价不等于最低每小时价格。

- 默认每 20 秒检查四店完整公开目录。请求超时、限流或字段结构不符时保留上次状态，单店指数退避至最多 10 分钟，不误判下架。
